...
**샌디스크가 한 주에 +26.98% 폭등**했다. 누적 +229.59%. 대단하다. 펀더멘털에 기반해서 이익이 증가하는 종목은 상승율이 압도적이다. 주가만 보면 작년 12월 진입가 31만원에서 지금 300만원, 6개월에 10배가 됐다. 분할 매수로 평단이 올라와 내 수익율은 +229%지만, 종목 자체는 10배 오른 셈이다.

종목을 잘 선정하고 잘 모아가면 충분히 수익율을 보장한다.

미장 비중이 30.77%에서 35.38%로 올라왔다. 샌디스크 한 종목이 끌어올렸다.

매수하는 종목들은 계속 빠진다. **현대모비스는 4거래일 연속 샀는데 -12.91%** 더 내렸다. 떨어질 때 분할로 사는 원칙은 지키고 있지만, 타이밍이 좋지 않았다. +57% 이익에서 -1.98% 내려왔다.
...
스페이스X 상장 이슈가 해결된 금요일 상승. 큰 종목이라 영향을 끼친다. 이익이 받쳐주지 않아 구매 계획은 없지만 시장은 반응한다. 시장의 반응을 배운다.
...
지수는 횡보인데 계좌만 올랐다. 메모리 비중이 큰 포트라 반도체가 움직이면 지수와 무관하게 출렁인다.
...
최근 한 달 매매 타이밍이 좋지 않다. 현대모비스는 4거래일 연속 샀는데 계속 빠졌고, 삼성전기도 편입하자마자 내렸다. 떨어질 때 분할로 사는 원칙은 맞다고 보는데, 사는 족족 더 내리니 차라리 날짜를 정해 기계적으로 소수점만 모으는 게 나을지 고민이 된다.

기계적으로 적용할 수 있게 국내 주식도 소수점 모으기를 적용해야겠다. 토스는 지원하지 않으니 삼성증권을 이용해봐야 겠다.
